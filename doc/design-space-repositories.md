...
1. **Raw nanopubs** of the following predefined types, loaded whole (all four graphs) and kept indefinitely:
   ...
   - `<https://w3id.org/kpxl/gen/terms/hasMaintainer>`
...
Trust seeding is per space ref, so in the rootless transition case each declaration becomes its own root and creates its own space ref. Multiple space refs for the same Space IRI are **distinct authority domains** and are deliberately *not* resolved to a canonical one: `dct:created` is publisher-asserted and forgeable, so any earliest- or latest-wins rule hands the IRI to whoever backdates (or out-dates) a rival root. Consumers treat refs sharing an IRI as distinct spaces claiming that IRI and display per ref. See `design-spaceref-isolation.md` for the per-ref authority isolation this implies. (An earlier revision of this paragraph prescribed first-root-wins by earliest `dct:created`; that rule is rejected as hijack-vulnerable.)
...
`NanopubLoader` runs `loadToSpacesRepo` for any nanopub that carries its own space-relevant extraction triples. Invalidators of space-relevant nanopubs are propagated to the spaces repo symmetrically — keeping the materialiser's invalidation join (`?invNp npx:invalidates ?np` plus `?invNp npa:hasLoadNumber ?ln` in `npa:graph`) populated regardless of load order, without polluting the repo with retractors of unrelated nanopubs (e.g. template-version supersedes chains):
- **Forward order** (target loaded before retractor): `loadInvalidateStatements`, while reading the target's `npx:hasNanopubType` from the meta repo to fan the retractor out into the target's per-type repos, also checks those types against `SpacesExtractor.TRIGGER_TYPES`. If any matches, the retractor's full content is loaded into the spaces repo too.
- **Reverse order** (retractor loaded before target): when this nanopub is space-relevant and `getInvalidatingStatements` returned prior retractors, `loadInvalidatorIntoSpacesRepo` reads each retractor's content from its per-pubkey repo and re-loads it into the spaces repo. This is the spaces-repo counterpart to `loadInvalidatorIntoTypeRepos` (PR #103).

`SpacesExtractor.TRIGGER_TYPES` is the union of the trigger IRIs the per-shape extractors dispatch on (`gen:Space`, `gen:hasRole`, `gen:SpaceMemberRole`, `gen:RoleInstantiation`, `gen:isSubSpaceOf`, `gen:MaintainedResource`, `gen:isMaintainedBy`, plus the 14 backcompat role predicates) — the loader's invalidation propagation checks against this constant so its definition of "space-relevant" doesn't drift from the extractor's.
