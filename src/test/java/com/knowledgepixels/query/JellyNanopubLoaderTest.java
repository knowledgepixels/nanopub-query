...
class JellyNanopubLoaderTest {
    ...
    void loadInitialWithAfterGreater() {
        try (MockedStatic<JellyNanopubLoader> mockedJellyLoader = mockStatic(JellyNanopubLoader.class, CALLS_REAL_METHODS)) {
            mockedJellyLoader.when(JellyNanopubLoader::fetchRegistryMetadata).thenReturn(new RegistryMetadata(5L, null, null, null, null));
    ...
    void loadInitialWithException() {
        try (MockedStatic<JellyNanopubLoader> mockedJellyLoader = mockStatic(JellyNanopubLoader.class, CALLS_REAL_METHODS)) {
            mockedJellyLoader.when(JellyNanopubLoader::fetchRegistryMetadata).thenReturn(new RegistryMetadata(10L, null, null, null, null));
